...
init python:
    class VignetteShader:
        def __init__(self, intensity=0.6):
            # Интенсивность виньетки, по умолчанию 0.6
            self.intensity = intensity

            # Определяем шейдер виньетки как строку GLSL
            self.shader_code = """
                uniform float intensity;
                
                vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
                    vec4 texcolor = texture2D(texture, texture_coords);
                    float dist = distance(texture_coords, vec2(0.5, 0.5));  // Расстояние от центра
                    float vignette = smoothstep(0.4, 0.8, dist);  // Параметры виньетки
                    return mix(texcolor, texcolor * (1.0 - intensity * vignette), intensity);
                }
            """

            # Компилируем шейдер
            self.shader = renpy.render.Shader(self.shader_code)

        def apply(self, image):
            """Метод для применения шейдера к изображению"""
            image.shader = self.shader
            image.shader_param("intensity", self.intensity)

        def set_intensity(self, intensity):
            """Метод для изменения интенсивности виньетки"""
            self.intensity = intensity
