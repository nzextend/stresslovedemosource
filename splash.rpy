init python:
    menu_trans_time = 1
    splash_message_default = "{font=/gui/Fonts03X/Pips.ttf}Продолжай идти вперед, если хочешь узнать, что такое отчаяние. \nНо не говори, что я тебя не предупреждала.{/font}"
    splash_messages = [
    "You are my sunshine,\nMy only sunshine",
    ]
...
image menu_nyx_art:
    subpixel True
    "gui/MenuArtNyx00.png"
    xcenter 550
    ycenter 410
    zoom 0.60
    menu_art_move(0.60, 550, 0.60)
...
image tos = "/images/warning01.png"
image tos2 = "/images/warning02.jpg"
...
label splashscreen:
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "StressLove - это неофициальная модификация для Doki Doki Literature Club и никак не связана с Team Salvato"
        "В этом моде так же может использоваться музыка из DDLC+, Minecraft, A Dance of Fire and Ice."
        "В данном моде могут содержаться сцены умеренного насилия, оскорбления, издевательства и социальное давление."
        menu:
            "И кстати, вы соглашаетесь на спойлеры из DDLC+, как там содержит музыку из нее, а так же из обычной DDLC." 
            "Если вы еще не прошли официальную игру, то советую её вам пройти."
            "Вы можете скачать её из itch.io (https://ddlc.moe), а так же у нас есть сайт: https://sites.google.com/view/stresscords/"
            "Ты согласен со всеми спойлерами, которые готов увидеть?"
            "Я согласен на спойлеры.":
                pass
...
label after_load:
    ...
    if persistent.yuri_kill > 0 and persistent.autoload == "yuri_kill_2":
        ...
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            ...
            call screen dialog("Подсказка: Испоьзуйте кнопку \"Пропуск\"  чтобы пропускать текст в моде.", ok_action=Return())
    return
